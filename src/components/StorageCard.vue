...
const props = defineProps<{ depot: z.infer<typeof Storage> }>();
const emit = defineEmits(["update"]);

const handleDelete = async () => {
  const confirmDelete = confirm("Möchten Sie dieses Depot wirklich löschen?");
  if (confirmDelete) {
    const result = await api(endpoints.deleteStorage, { params: { id: props.depot.id } });
    match(result, {
      ok: () => {
        notification.success({
          message: "Erfolg",
          description: `Depot ${props.depot.id} gelöscht!`,
          duration: 3,
        });
        emit("update");
      },
      error: errors => notification.error({
        message: "Fehler",
        description: `Depot konnte nicht gelöscht werden: ${errors.map(err => err.message).join(", ")}`,
        duration: 7,
      }),
    });
  }
  ...
  >
    ...
    <div class="h-[2.5rem] w-full flex flex-row items-stretch self-end border-t border-dark-1 opacity-0 transition-all group-hover:opacity-100">
      <button class="w-full overflow-hidden border-r border-dark-1 transition-colors">
        ...
        <RouterLink :to="`/depot/${props.depot.id}`">
